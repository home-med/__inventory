...
DROP TRIGGER IF EXISTS "trg_insert_product_update_product_visibility";
CREATE TRIGGER trg_insert_product_update_product_visibility AFTER INSERT ON product
BEGIN
	INSERT INTO product_visibility (item, location, eCom, archived)
	SELECT NEW.id, id, false, false FROM location;
END;
COMMIT;
